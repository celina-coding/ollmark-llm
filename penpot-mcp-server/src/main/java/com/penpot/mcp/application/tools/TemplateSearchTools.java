...
public class TemplateSearchTools {
        ...
        try {
            ...
            if (templates.isEmpty()) {
                return String.format(
                    "{\"message\": \"No templates found for query: %s\", \"suggestions\": %s}",
                    escapeJson(query),
                    listAvailableTypes()
                );
            }
    ...
    public String generateFromTemplate(
        ...
        try {
            ...
            // Return as JSON with the code
            return String.format(
                "{\"templateId\": %s, \"code\": %s, \"type\": %s}",
                escapeJson(templateId),
                escapeJson(code),
                escapeJson(template.getType())
            );
            
        } catch (IllegalArgumentException e) {
            log.warn("Template not found: {}", templateId);
            return String.format("{\"error\": \"Template not found: %s\"}", 
                    escapeJson(templateId));
        } catch (Exception e) {
    ...
    public String getTemplatesByType(
        ...
        try {
            ...
            if (templates.isEmpty()) {
                return String.format(
                    "{\"message\": \"No templates found for type: %s\", \"availableTypes\": %s}",
                    escapeJson(type),
                    listAvailableTypes()
                );
            }
